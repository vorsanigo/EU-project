...
# 2) Framework Programme on total data
...
# set output dir according to data sampl    e
...
if red_emb_type == 'umap':
    output_dir = 'min spanning tree CORRECT/knn null model ' + sampling + ' norm ' + str(components) + ' umap/'
elif red_emb_type == 'pca':
    output_dir = 'min spanning tree CORRECT/knn null model ' + sampling + ' norm ' + str(components) + ' PCA/'
elif red_emb_type == 'no_red_emb':
    output_dir = 'min spanning treee bge tot spece CORRECT/knn null model ' + sampling + ' norm tot emb/' 
...
if mst_type == 'window tot':
    ...
    for i in range(num_iter):
        ...
        if i == 0:
            ...
            #sum_arrays = np.add(sum_arrays, length_list)
...
# 3) FP on total data
    ...
    for j in range(num_iter):
        ...
        for i in range(len(fp_list)):
            ...
            fp_df = proj_emb_df.filter(
                (pl.col("fp") == fp_list[i]) &
                (pl.col(embeddings_col).is_not_null()) #TODO ADDED HERE THIS ROW
            )

            print("FP: ", str(i+1))
            print("# projects: ", str(len(fp_df)))

            fp_df = fp_df.unique(subset=['id'])
            print('# projects no duplicates embeddings:', len(fp_df))
            print('# projects no duplicates proj id:', len(fp_df.unique(subset=['id'])))

            # extract points from embeddings
            points_real = [np.array(x) for x in fp_df[embeddings_col].to_list()]
            points_real = np.array(points_real, dtype=float)
            ...
            if len(points) > points_threshold: #TODO ADDED THIS ROW
    ...
    mean_length_list = np.mean(lengths_arrays, axis=0)
    std_length_list = np.std(lengths_arrays, axis=0)    
    ...
    fig,ax = plt.subplots(figsize = (9,9))
    ax.plot(fp_plot_list, mean_length_list, '-o') # TODO CHANGED FROM fp_name_list TO fp_plot_list
    ax.fill_between(fp_plot_list, mean_length_list - std_length_list, mean_length_list + std_length_list, color="orange", alpha=0.2, label="±1 std")
    ax.tick_params(axis='both', which='major', labelsize=16)
    ax.set_xlabel('Time', fontsize=20)
    ax.set_ylabel('MST length', fontsize=20)
    plt.show()

    fig.savefig(output_plot_path + 'FP_tot_mean.png')
