...
# embeddings
emb = np.load('embeddings_topics/description embeddings/bge_1024dim.npy')
...
if (mst_type == 'window tot') or (mst_type == 'window country'):
    if red_emb_type == 'umap':
        output_dir = 'min spanning tree/knn norm' + str(time_window) + ' year_months step ' + str(step) + ' window/' + str(components) + ' umap/'
    elif red_emb_type == 'pca':
        output_dir = 'min spanning tree/knn norm' + str(time_window) + ' year_months step ' + str(step) + ' window/' + str(components) + ' PCA/'
    elif red_emb_type == 'no_red_emb':
        output_dir = 'min spanning tree/knn norm' + str(time_window) + ' year_months step ' + str(step) + ' window/tot emb NORM CUMULATIVE/'

elif (mst_type == 'FP tot') or (mst_type == 'FP country'):
    if red_emb_type == 'umap':
        output_dir = 'min spanning tree/FP/knn norm ' + str(components) + ' umap/'
    elif red_emb_type == 'pca':
        output_dir = 'min spanning tree/FP/knn norm ' + str(components) + ' PCA/'
    elif red_emb_type == 'no_red_emb':
        output_dir = 'min spanning tree/FP/knn norm tot emb/'
...
if mst_type == 'window tot':
    ...
    while current_end <= end_date:
        ...
        current_end += relativedelta(months=window_size)
        ...
        if len(points) > points_threshold:
            ...
            if mst != None:
                ...
                #length_list += [mst.sum()]
                windows_list += [current_end - relativedelta(months=step)] 
    ...
    fig.savefig(output_plot_path + 'window_tot.png')
...
elif mst_type == 'window country':

    # output dir
    #output_dir = 'min spanning tree/knn norm/'
